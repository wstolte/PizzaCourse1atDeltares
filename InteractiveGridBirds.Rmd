...
```{r, message=F, warning=F, comment=F, echo = F, cache = T}
...
mainPanel(
  renderPlot({
    ...
    map = map + geom_point(data = birds_selected, aes(x.RD, y.RD, color = log10(Numeriekewaarde)), alpha = 0.8, shape = 20, size = 1)
    if(input$type == "rect")  {
