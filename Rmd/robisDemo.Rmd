...
```{r plotLeafletmap}
df <- occurrence(scientificname = c("Dinophysis"), geometry = "POLYGON ((2 50,2 55, 10 55, 10 50, 2 50))")
...
leaflet(df) %>%
  ...
  addCircleMarkers(lat = ~decimalLatitude, 
                   ...
                   label = ~paste(scientificName, yearcollected)
                   ) %>%
  addLayersControl(
...
ggplot(data = df, aes(decimalLongitude, decimalLatitude)) +
  ...
  coord_map(projection = "ortho",
            ...
            xlim =  expand_scale(range(data$decimalLongitude), c(-4,10)) ,
            ylim = expand_scale(range(data$decimalLatitude), c(50, 60))
            )
```
...
df %>% 
  group_by(scientificName, yearcollected) %>%
  summarize(sum = n()) %>%
  summary()
...
df %>% 
  group_by(scientificName, yearcollected) %>%
  summarize(sum = n()) %>%
  ggplot (aes(x = yearcollected, y = sum)) + 
  geom_boxplot(aes(group = yearcollected), alpha = 0.2) +
